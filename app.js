...
(function() {
  ...
  // ---------- 0. CONFIGURACIÓN DE COLORES (PRIMERO) ----------
  ...
  // ============================================================
  // APLICAR COLORES DESDE CONFIG_BODA
  // ============================================================
  function aplicarColoresDesdeConfig() {
    if (!CONFIG || !CONFIG.colores) {
      console.warn("⚠️ No se encontraron colores en CONFIG");
      return;
    }
    ...
    // Mapeo directo de CONFIG a variables CSS
    root.style.setProperty('--color-primario', c.primario || '#C9A84C');
    root.style.setProperty('--color-secundario', c.secundario || '#B8942E');
    root.style.setProperty('--color-acento', c.acentoOcasional || '#D4AF37');
    root.style.setProperty('--color-fondo', c.fondoContraste || '#F5EDE0');
    
    // Colores fijos (no vienen de CONFIG)
    root.style.setProperty('--color-texto', '#8B7A5A');
    root.style.setProperty('--color-claro', '#FFFCF5');
    
    console.log("🎨 Colores aplicados desde CONFIG:", {
      primario: c.primario,
      secundario: c.secundario,
      acento: c.acentoOcasional,
      fondo: c.fondoContraste
    });
  }

  // Ejecutar colores ANTES de cualquier otra cosa
  aplicarColoresDesdeConfig();
...
// Elementos
const entrySection = document.getElementById("sceneEntry");
const entryBtn = document.getElementById("entryBtn");
const sceneRevelation = document.getElementById("sceneRevelation");

// Función de entrada
function entrar() {
  console.log("🎯 Entrando a la invitación...");
  
  // Animación de salida de la entrada
  if (entrySection) {
    entrySection.classList.add("exit");
  }
  
  // Agregar clase open a las flores si existen
  if (sceneRevelation) {
    sceneRevelation.classList.add("open");
  }
  
  // Activar scroll y música después de la animación
  setTimeout(() => {
    ...
    // SCROLL AUTOMÁTICO A LA SECCIÓN HERO
    const heroSection = document.getElementById("sceneHero");
    if (heroSection) {
      heroSection.scrollIntoView({ 
        behavior: 'smooth', 
        block: 'start' 
      });
      console.log("📜 Scroll automático a la sección Hero");
    }
    ...
    if (audio && !window.isMusicPlaying) {
      audio.play().then(() => {
...
// Evento: click en el botón o en toda la sección
if (entryBtn) {
...
if (entrySection) {
  entrySection.addEventListener("click", function(e) {
    if (e.target.closest('.entry-btn')) return;
    entrar();
  });
}

console.log("🎬 Entrada elegante inicializada - Scroll bloqueado");
  ...
  console.log("🏁 App inicializada correctamente - Scroll bloqueado hasta abrir la revelación");
  ...
  function initGoogleCalendar() {
    setTimeout(() => {
      ...
      nuevoBtn.addEventListener('click', (e) => {
        ...
        const title = encodeURIComponent(` ${CONFIG.novios.el} y ${CONFIG.novios.ella}`);
...
(function() {
  ...
  // ... (todo tu código existente: setText, setHtml, imagenes, itinerario, etc.) ...

 // ============================================================
// FONDO DINÁMICO CON ÍCONOS - VERSIÓN SIMPLIFICADA
// ============================================================
function initFondoDinamico() {
  const container = document.getElementById('floatingBackground');
  if (!container) return;

  const config = CONFIG.fondoDinamico || {};
  
  const iconosDisponibles = {
    nube: 'fa-cloud',
    nube_sol: 'fa-cloud-sun',
    estrella: 'fa-star',
    sol: 'fa-sun',
    luna: 'fa-moon',
    flor: 'fa-seedling',
    corazon: 'fa-heart',
    conejo: 'fa-rabbit',
    mariposa: 'fa-butterfly',
    sparkle: 'fa-sparkles'
  };

  const iconoSeleccionado = config.icono || 'nube';
  const iconoClase = iconosDisponibles[iconoSeleccionado] || 'fa-cloud';
  
  const cantidad = config.cantidad || 14;
  const velocidad = config.velocidad || 20;
  const tamaño = config.tamaño || '3rem';
  const opacidad = config.opacidad || 0.1;
  const color = config.color || 'var(--color-primario)';
  ...
  for (let i = 0; i < cantidad; i++) {
    ...
    item.classList.add(esIzquierda ? 'float-left' : 'float-right');
    
    const icon = document.createElement('i');
    icon.className = `fas ${iconoClase}`;
    
    const sizeMultiplier = 0.5 + Math.random() * 0.8;
    const finalSize = parseFloat(tamaño) * sizeMultiplier;
    
    // Posiciones COMPLETAMENTE ALEATORIAS en la pantalla
    const startX = Math.random() * 100;
    const startY = Math.random() * 100;
    const delay = Math.random() * velocidad;
    const floatY = (Math.random() - 0.5) * 120;
    const rotation = (Math.random() - 0.5) * 60;
    const duration = velocidad + (Math.random() * 10);
    
    item.style.cssText = `
      top: ${startY}%;
      left: ${startX}%;
      font-size: ${finalSize}rem;
      --float-y: ${floatY}px;
    ...
    icon.style.cssText = `
      font-size: inherit;
      filter: drop-shadow(0 2px 12px rgba(201, 168, 76, 0.05));
    `;
  ...
  console.log(`☁️ Fondo dinámico iniciado: ${cantidad} íconos de tipo "${iconoSeleccionado}"`);
}

  // ✅ EJECUTAR EL FONDO DINÁMICO
  initFondoDinamico();
